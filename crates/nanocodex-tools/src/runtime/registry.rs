...
impl ToolRegistry {
    ...
    pub(crate) fn code_mode_tool_names(&self) -> Vec<(String, String)> {
        let mut names = self
            ...
            .map(|definition| {
                (
                    super::selection::normalize_public_tool_name(definition.name()),
    ...
    pub(crate) fn code_mode_tool_summaries(&self) -> Vec<(String, String)> {
        let mut seen = self
            ...
            .into_iter()
            .map(|definition| super::selection::normalize_public_tool_name(definition.name()))
            .collect::<HashSet<_>>();
        let mut summaries = self
            ...
            .filter_map(|(name, description)| {
                let normalized = super::selection::normalize_public_tool_name(&name);
...
fn first_normalized_definitions(
    ...
    definitions
        .into_iter()
        .filter(|definition| {
            let normalized = super::selection::normalize_public_tool_name(definition.name());
...
fn definition_metadata(name: &str, definition: &ToolDefinition) -> Value {
    ...
    let metadata_name = super::selection::normalize_public_tool_name(name);
    json!({
        "name": metadata_name,
        "tool_name": name,
