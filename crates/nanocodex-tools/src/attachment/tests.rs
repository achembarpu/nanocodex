...
#[tokio::test]
async fn catalog_rejection_does_not_trust_a_mispinned_fence_reason() {
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let endpoint = format!("ws://{}/attach", listener.local_addr().unwrap());
    let server = tokio::spawn(async move {
        let (stream, _) = listener.accept().await.unwrap();
        ...
        send_json(
            ...
            json!({
                "type":"fenced","protocol_version":1,"capability":"tools",
                "lease_id":lease_id,"generation":2,"reason":"untrusted remote reason"
            }),
        )
        .await;
        let _ = socket.next().await;
    ...
    let error = tools
        ...
        .unwrap_err();
    assert!(
        matches!(error, AttachmentError::Fenced(reason) if reason.as_ref() == "fence pin did not match active lease")
    );
    server.await.unwrap();
}
