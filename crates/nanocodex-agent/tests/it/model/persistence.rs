...
struct FailAppendOnce {
    inner: nanocodex_agent::durability::MemoryStore,
    expected_revision: u64,
    failed: Arc<std::sync::atomic::AtomicBool>,
}

impl nanocodex_agent::durability::JournalStore for FailAppendOnce {
    fn load<'a>(
        &'a mut self,
        journal_id: &'a str,
    ) -> nanocodex_agent::durability::StoreFuture<
        'a,
        std::result::Result<
            nanocodex_agent::durability::StoredJournal,
            nanocodex_agent::durability::StoreError,
        >,
    > {
    ...
    fn append<'a>(
        ...
        'a,
        std::result::Result<u64, nanocodex_agent::durability::StoreError>,
    > {
        if expected_revision == self.expected_revision
            && !self.failed.swap(true, std::sync::atomic::Ordering::SeqCst)
        {
            return Box::pin(async {
                Err(nanocodex_agent::durability::StoreError::NotCommitted(
...
-> Result<()> {
    ...
    let journal =
        nanocodex_agent::durability::DurableSession::open(failing_store, "portable-model-replay")
            .await?;
    ...
    let journal =
        nanocodex_agent::durability::DurableSession::open(store, "portable-model-replay").await?;
    let builder = Nanocodex::builder(openai()?)
        .workspace(&workspace)
        .session_id(test_session_id())
        .durability(journal)
...
async fn portable_journal_restores_the_live_owner_before_retrying_a_failed_commit() -> Result<()> {
    ...
    let journal = nanocodex_agent::durability::DurableSession::open(
        failing_store,
        "portable-model-live-retry",
    )
    .await?;
    let builder = Nanocodex::builder(openai)
        .workspace(&workspace)
...
async fn portable_journal_refuses_to_repeat_a_tool_with_an_ambiguous_completion() -> Result<()> {
    ...
    let journal =
        nanocodex_agent::durability::DurableSession::open(failing_store, "ambiguous-tool").await?;
    let builder = Nanocodex::builder(openai()?)
        .workspace(&workspace)
        .session_id(test_session_id())
        .tools(tools()?)
    ...
    let journal =
        nanocodex_agent::durability::DurableSession::open(store, "ambiguous-tool").await?;
    let builder = Nanocodex::builder(openai()?)
        .workspace(&workspace)
        .session_id(test_session_id())
        .tools(tools()?)
