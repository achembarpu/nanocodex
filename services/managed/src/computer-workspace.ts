...
/** The filesystem-only part of @cloudflare/computer used by Nanocodex. */
export type ComputerWorkspaceClient = Readonly<{
  fs: {
...
type WorkspaceClientIsCompatible = WorkspaceClient extends ComputerWorkspaceClient ? true : false;
type Assert<T extends true> = T;
type _AssertWorkspaceClientIsCompatible = Assert<WorkspaceClientIsCompatible>;

/**
 * Adapts Cloudflare Computer's durable SQLite filesystem to Nanocodex's
 * host-generic workspace contract. No Computer execution backend is installed:
 * callers mount the returned handle into the bounded Just Bash runtime.
 */
export async function createComputerFilesystem(
  workspace: ComputerWorkspaceClient,
...
async function ensureRoot(fs: ComputerWorkspaceClient["fs"]): Promise<void> {
