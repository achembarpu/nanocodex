...
describe("per-user credential broker", () => {
  ...
  it("connects OpenRouter with S256 PKCE, keeps the subscription default, and allows explicit selection", async () => {
    const user = "user-openrouter-choice";
    const subject = "O".repeat(43);
    await control(`/subjects/${subject}`, "PUT", { user_id: user });
    await SELF.fetch(`https://broker.test/users/${user}/credentials/chatgpt/login`, {
      method: "POST",
    });
    await SELF.fetch(`https://broker.test/users/${user}/credentials/chatgpt/login/status`, {
      method: "POST",
    });

    const started = await SELF.fetch(
      `https://broker.test/users/${user}/credentials/openrouter/login`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          callback_url: "http://nanocodex.localhost:5197/v1/credentials/openrouter/callback",
        }),
      },
    );
    expect(started.status).toBe(200);
    const authorization = new URL((await started.json<{ authorization_url: string }>()).authorization_url);
    expect(authorization.origin + authorization.pathname).toBe("https://openrouter.ai/auth");
    ...
    expect(callback.origin + callback.pathname).toBe(
      "http://nanocodex.localhost:5197/v1/credentials/openrouter/callback",
    );
    expect(callback.searchParams.get("state")).toMatch(/^[A-Za-z0-9_-]{43}$/);

    const connected = await SELF.fetch(
      `https://broker.test/users/${user}/credentials/openrouter/callback`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          code: "openrouter-authorization-code",
          state: callback.searchParams.get("state"),
        }),
      },
    );
    expect(connected.status).toBe(204);
    const defaultStatus = await (await SELF.fetch(
      `https://broker.test/users/${user}/credentials`,
    ...
    expect(JSON.stringify(defaultStatus)).not.toMatch(/sk-or|code_challenge|openrouter-authorization-code/);

    expect((await control(`/users/${user}/credentials/active`, "PUT", {
      provider: "openrouter",
    })).status).toBe(204);
    const websocket = await SELF.fetch(new Request("https://nanocodex.internal/v1/responses", {
      headers: {
        authorization: "Bearer NANOCODEX_PROVIDER_CREDENTIAL",
        "openai-beta": "responses_websockets=2026-02-06",
        upgrade: "websocket",
    ...
    const response = await SELF.fetch(new Request("https://nanocodex.internal/v1/responses", {
      method: "POST",
      headers: {
        accept: "text/event-stream",
        authorization: "Bearer NANOCODEX_PROVIDER_CREDENTIAL",
        "content-type": "application/json",
        "session-id": "openrouter-session",
        "x-nanocodex-subject": subject,
      },
      body: JSON.stringify({
        model: "gpt-5.6-sol",
        input: "hello",
        stream: true,
        client_metadata: { session_id: "openrouter-session" },
      }),
    }));
    expect(response.status).toBe(200);
    expect(response.headers.get("authorization")).toBeNull();
    expect(await response.text()).toContain(
      '"observed_model":"openai/gpt-5.6-sol","observed_session":"openrouter-session","authorized":true',
    );
    ...
    expect(await (await SELF.fetch(
      `https://broker.test/users/${user}/credentials`,
    )).json()).toMatchObject({ active: "chatgpt" });
  });
