...
| D-44 persistent host schemas could look compatible while changing authority semantics | Native and JavaScript Postgres validation now pins schema, column types/defaults/nullability, exact CHECKs, primary keys, immediate same-schema foreign keys, and the absence of extra constraints before any mutation | PGlite/native schema suites covering cross-schema and deferred FKs, missing/extra CHECKs, mixed legacy counters, and counter exhaustion |
