...
`nanocodex-terminal` sibling packages under `..`; it does not reach into generated
WASM artifacts. Its
...
[Cloudflare Worker example](../managed/README.md#multiplayer-managed-agent-rooms)
...
- `../nanocodex` publishes `nanocodex`, the viem-v3-style imperative client.
...
- `../nanocodex-react` publishes `nanocodex-react`, the wagmi-like headless React owner. Its provider and
  hooks manage the module Worker lifecycle, readiness, commands, and event
  subscriptions without imposing presentation policy.
- `../nanocodex-terminal` publishes `nanocodex-terminal`, the controlled transcript
...
Run `npm run bench:dataset` in `js/nanocodex` for the deterministic 100,000-row
