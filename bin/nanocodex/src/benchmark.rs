...
5. Unless pressure is elevated or a worker just vanished, wait 25 seconds before the next compact observation so useful work can complete within one host-tool call. Let existing workers drain under overload; never stop, signal, or shed one manually.
