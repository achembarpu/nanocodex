...
3. Reason from the current and recent observations and choose an absolute desired live-worker count that maximizes terminal completions per hour. Starting units count as live. Enter recovery mode whenever memory `full avg10` is nonzero or `recent_attempts.failures` is nonempty: admit nothing and let existing workers drain until both signals clear. Outside recovery mode, with backlog and no measured overload or throughput stall, grow aggressively in a batch; unused healthy capacity is a controller failure. OOMs and infrastructure retries are acceptable calibration signals, but repeatedly replacing them before recovery is a controller failure. High CPU or load alone is not overload.
