...
1. Observe, in a fresh Code Mode call, `{status_command}`, live or activating `nanocodex-eval-worker-*.service` user units, `/proc/meminfo`, `/proc/loadavg`, `/proc/pressure/memory`, swap activity, and recent completions or worker exits. Do not keep a JavaScript loop, PID marker, worker pool, or other controller state.
...
Controller failure or restart must leave every worker untouched. On restart, derive the complete situation again from SQLite and systemd. Do not use subagents to own workers and do not wait for worker processes in Code Mode. Finish only when status has zero unclaimed and running rows and no live worker unit remains."#,
    )
}
