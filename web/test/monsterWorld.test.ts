...
test("the World is one Rust subagent task tree with one embodied session per resident", () => {
  ...
  assert.match(worker, /for \(let attempt = 0; attempt < 2; attempt \+= 1\)[\s\S]*?agent\.turn\.prompt\([\s\S]*?setupCorrectionPrompt\(active, correction\)[\s\S]*?const result = await turn\.result\(\)/);
  assert.match(worker, /Return exactly one JSON object and do not call tools/);
  assert.doesNotMatch(worker, /submit_choreography|WORLD_PLAN_PARAMETERS/);
  ...
  assert.match(worker, /latest\.result\.outcome\.status !== "completed"/);
  ...
  assert.match(worker, /const plan = await planWorldSetup\(agent, active\)/);
  assert.match(worker, /return parseWorldPlan\(active, result\.finalMessage\)/);
  assert.match(worker, /failureClass\(cause\) !== "invalid"/);
  assert.match(worker, /function setupCorrectionPrompt/);
  assert.match(worker, /for \(const phase of plan\)[\s\S]*?active\.setup = setup;[\s\S]*?active\.placements = assignFormationRegions\(active, setup\);[\s\S]*?await dispatchInitialWave\(active, setup\);[\s\S]*?if \(!residentTreeReady\)[\s\S]*?active\.runtimeOwnedPlan[\s\S]*?assertRetainedResidents\(active\)[\s\S]*?dispatchResidents\(agent, active, setup\)[\s\S]*?settleFormationAfterInitialWave\(active, setup\)[\s\S]*?else \{\s*await settleFormationAfterInitialWave\(active, setup\)/);
  ...
  assert.match(worker, /async function settleFormationAfterInitialWave[\s\S]*?outcome\.status !== "completed"/);
  assert.match(worker, /wave < 32/);
  assert.match(worker, /latestFormationSnapshot\(active\)/);
  assert.match(worker, /phaseIsHolding\(active, setup, snapshot\.waves\)/);
  assert.match(worker, /Promise\.all\(\[\.\.\.unfinished\]\.map/);
  assert.match(worker, /commitHoldingSnapshot\(active, snapshot\)/);
  assert.match(worker, /await interruptResidentTurns\(agent, active\);\s*lastCompletedChoreography/);
  assert.doesNotMatch(worker, /Subagents\.wait/);
  assert.match(worker, /lastCompletedChoreography = completedChoreography\(active, setup\)/);
  assert.match(worker, /World reducer finished without holding evidence from/);
  assert.match(worker, /active\.liveRoster = preceding\.roster/);
  assert.match(worker, /active\.liveRoster \?\? active\.entry\.observation\.roster/);
  assert.match(worker, /function retainedScalePlan/);
  assert.match(worker, /if \(retainedScale\) \{\s*active\.runtimeOwnedPlan = true;\s*return retainedScale/);
  assert.match(worker, /function assertRetainedResidents/);
  assert.match(worker, /extentPixels: clampExtent\(formation\.extentPixels \* transform\)/);
  assert.match(worker, /formation\.layout === undefined \? \{\} : \{ layout: formation\.layout \}/);
  assert.match(worker, /startResidentTasks\(agent, active, fresh\)/);
  assert.match(worker, /Subagents\.spawnMany\(agent, tasks\.map/);
  assert.match(worker, /Subagents\.send\(agent, \{[\s\S]*?purpose: "delegate"/);
  assert.match(worker, /const failed = retained\.filter[\s\S]*?Subagents\.close[\s\S]*?startResidentTasks\(agent, active, failed\)/);
  assert.match(worker, /starts every resident exactly once for Scout's whole command/);
  assert.match(worker, /function joinWorldAction/);
  assert.match(worker, /pending\.completion\.then\(resolve, reject\)/);
  assert.match(worker, /groupOrdinal: active\.placements\?\.get\(residentId\)\?\.regionIndex \?\? -1/);
  assert.match(worker, /clearRegionMarket\(\s*unassignedMembers,\s*openComponents/);
  assert.match(worker, /active\.completedFormations\.get\(component\.formation\.id\)/);
  assert.match(worker, /const retainedComponents = new Set\(retainedMembers\.map/);
  assert.match(worker, /const openComponents = componentRegions\.filter/);
  assert.match(worker, /clearRegionMarket\(localStates, regions/);
  assert.match(worker, /kind: "maintain_formation"/);
  assert.match(worker, /path_tiles: placement\.pathTiles/);
  assert.match(worker, /const formationId = active\.placements\?\.get\(expectedMembers\[0\]\)\?\.formationId/);
  assert.match(worker, /candidate\.formationId === formationId/);
  assert.doesNotMatch(worker, /deconflictFormationSlots|separateFormationComponents/);
  assert.match(worker, /function plannedPositionAction/);
  assert.match(worker, /function formationTilePath/);
  assert.match(worker, /component \* extentPixels/);
  assert.match(worker, /coalesceIdenticalFormations\(resolveFormationLayouts\(parsedFormations\)\)/);
  assert.match(worker, /leaders: Object\.freeze\(\[\.\.\.previous\.leaders, \.\.\.formation\.leaders\]\)/);
  assert.match(worker, /A layout is only for 2\+ repeated components, never one contour/);
  assert.match(worker, /emit exactly one formation whose leaders array contains every active leader once/);
  assert.match(worker, /Task text does not position anything/);
  assert.match(worker, /Never repeat a leader in two formations in one phase/);
  assert.match(worker, /anchor_placement/);
  assert.match(worker, /function resolveFormationPaths/);
  assert.match(worker, /formation relation cycle includes/);
  assert.match(worker, /currentIds\.has\(formation\.relativeTo\.formationId\)/);
  assert.match(worker, /function placeFormationAtAnchor/);
  assert.match(worker, /formation\.anchorPlacement/);
  assert.match(worker, /const layout = isSingleComponentLayout\(/);
  assert.match(worker, /composeFormationPath\(formation\.path/);
  assert.match(worker, /anchor: formation\.anchor,[\s\S]*closed: formation\.closed,[\s\S]*path: formation\.path/);
  assert.match(worker, /function orderedFormations[\s\S]*?new Set\(setup\.values\(\)\)/);
  assert.match(worker, /active\.addressed\.has\(anchor as ResidentId\)/);
...
test("the World surface stays statically available, stoppable, and semantically observable", () => {
  ...
  assert.match(component, /message\.failure === "invalid"[\s\S]*?Number\.POSITIVE_INFINITY/);
  assert.match(component, /const speech = playerSpeak[\s\S]*?setAgentError\(undefined\)/);
