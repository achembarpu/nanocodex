...
test("the World is one Rust subagent task tree with one embodied session per resident", () => {
  ...
  assert.match(worker, /delegate the exact task JSON only when task\.reviewer=true/);
  ...
  assert.match(worker, /for \(const phase of plan\)[\s\S]*?active\.setup = setup;[\s\S]*?active\.placements = assignFormationRegions\(active, setup\);[\s\S]*?await dispatchUntilFormationSettles\(active, setup\);[\s\S]*?dispatchResidents\(agent, active, setup\)/);
  ...
  assert.match(worker, /async function dispatchUntilFormationSettles[\s\S]*?outcome\.status !== "completed"/);
  ...
  assert.match(worker, /lastCompletedChoreography = completedCoordinationHistory\(active\)/);
  assert.match(worker, /agent\.subagents\.startMany\(fresh\.map/);
  assert.doesNotMatch(worker, /agent\.subagents\.start\(/);
  assert.match(worker, /runtime dispatches the complete provisional region-claim wave immediately after setup/);
  assert.match(worker, /groupOrdinal: active\.placements\?\.get\(residentId\)\?\.regionIndex \?\? -1/);
  assert.match(worker, /clearRegionMarket\(memberStates, componentRegions/);
  ...
  assert.match(worker, /A layout is only for 2\+ repeated components; omit it for one contour/);
  assert.match(worker, /const layout = isSingleComponentLayout\(rawLayout\) \? undefined : rawLayout/);
  assert.match(worker, /\.count === 1[\s\S]*?\.index === 0/);
