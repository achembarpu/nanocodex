...
import {
  ...
  WebSocketStream,
  type SshChannelClosedEventArgs,
...
export function createSshCommand(filesystem: IFileSystem) {
...
async function executeSsh(
  ...
  try {
    ...
    const closed = new Promise<SshChannelClosedEventArgs>((resolve) => channel.onClosed(resolve));
    ...
    if (websocket.readyState === WebSocket.OPEN) websocket.close(1000, "command complete");
  }
}

async function authenticateHost(
  publicKey: { getPublicKeyBytes(): Promise<Buffer | null> },
...
function parseArguments(args: string[]): SshArguments | { error: string } {
  if (!args.length || args.includes("--help")) return { error: usage() };
  let username = "";
  let identityFile = "";
  ...
  for (; index < args.length; index += 1) {
    ...
    if (arg === "-l" || arg === "-i" || arg === "-o") {
      ...
      else if (value.startsWith("HostKeySHA256=")) hostKeySha256 = value.slice("HostKeySHA256=".length);
      else return { error: `ssh: unsupported browser option '${value}'` };
      continue;
    }
    return { error: `ssh: unsupported browser option '${arg}'` };
  }
  const endpointText = args[index++];
  if (!endpointText) return { error: usage() };
  let endpoint: URL;
  try { endpoint = new URL(endpointText); } catch { return { error: "ssh: endpoint must be a ws:// or wss:// URL" }; }
  ...
  if (!username) return { error: "ssh: -l USER is required" };
  if (!identityFile) return { error: "ssh: -i PRIVATE_KEY is required" };
  ...
  if (!commandArgs.length) return { error: "ssh: an explicit remote command is required (interactive PTYs are unavailable)" };
  return {
    endpoint,
    username,
    identityFile,
...
function waitForWebSocket(websocket: WebSocket, signal?: AbortSignal): Promise<void> {
