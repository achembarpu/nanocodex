...
test("Responses multiplexer keeps logical channel rejection inside the upgraded socket", async () => {
  const originalFetch = globalThis.fetch;
  const OriginalResponse = globalThis.Response;
  const OriginalWebSocketPair = (globalThis as any).WebSocketPair;
  let upstreamDialed = false;
  ...
  class FakeWorkerSocket {
    ...
    close(code = 1000, reason = "") {
      ...
      for (const listener of this.peer?.listeners.get("close") ?? []) listener({ code, reason });
    }
  }
  class WorkerTestResponse extends OriginalResponse {
    webSocket: WebSocket | null = null;
  ...
  try {
    const response = await worker.fetch(new Request(
      "https://demo.test/api/responses/mux",
      {
        headers: {
          origin: "https://demo.test",
          upgrade: "websocket",
    ...
    assert.equal(response.status, 101);
    const client = response.webSocket as unknown as FakeWorkerSocket;
    client.send(JSON.stringify({
      type: "nanocodex.mux.open",
      channel_id: "c1",
      session_id: "session-1",
    }));
    await waitFor(() => client.messages.length >= 1);
    assert.deepEqual(JSON.parse(client.messages[0] ?? "null"), {
